...
Reproducible in :mod:`lino_book.projects.cosi_ee`. Select organization "Garage
...
entries (one per partner) no longer depends on Account.needs_partner but on
whether the journal has a partner or not.
...
I didn't yet commit my changes because the test suites will require revision.
