...
We must probably replace :mod:`lino.modlib.memo.utils` by markdown.  The editor must
have a list of users (to expand ``@``) and a list of tickets (to expand ``#``).
My :mod:`lino.modlib.memo.utils` parser is a nice DIY gadget, but it has no future
