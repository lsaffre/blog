...
parser.add_argument('-i', '-I', '--input', nargs='+', help='Input video file')
parser.add_argument('-o',
                    '-U',
                    '--output',
                    default="combine_result.avi",
...
def Fram_connect(frame1, frame2, w1, h1, w2, h2):
    frame2 = cv2.resize(frame2, (int(w2), int(h1)),
                        interpolation=cv2.INTER_AREA)
    BG = cv2.resize(frame1, (int(w1 + w2), int(h1)),
                    interpolation=cv2.INTER_AREA)
