...
3. cd to the root of a repository and run the :cmd:`inv initdb`
...
- If we look at the code of :mod:`atelier.invlib` we can see the
  :cmd:`inv initdb` command sets the current directory as follows::
...
- The :cmd:`inv initdb` command now always runs in the
