...
- The default `clone_row` method (which does nothing) is now defined on 
  :class:`lino.code.modeltools.Model`, and :func:`lino.core.kernel.analyze_models` 
  now injects this to models that don't have it.
  :meth:`lino.mixins.clonable.Clonable.clone_row` 
  no longer tests whether related slaves are an instance 
  of :mod:`lino.mixins.clonable.Clonable` before calling their on_duplicate.
