...
class Journal(models.Model):
    ...
    def get_next_number(self):
        self.save()
        cl = self.get_doc_model()
        d = cl.objects.filter(journal=self).aggregate(models.Max('number'))
...
class TradingVoucher(JournaledAbstractDocument):
...
class Order(TradingVoucher):
    valid_until = models.DateField(auto_now=True)


class Invoice(LedgerDocument, TradingVoucher):
