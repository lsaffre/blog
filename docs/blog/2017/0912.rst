...
I found the solution for :ticket:`1296`!  The problem occurred when
appyPod spawned a Python3 subprocess using its `pythonWithUnoPath`
setting.  The problem occurred only when running our test using
...
imported by a Python that didn't install it).

:meth:`atelier.utils.SubProcessParent.build_environment` now
explicitly removes :envvar:`PYTHONPATH` from the environment of the
child process.
      
I have no complete explanation yet.  Does :command:`setup.py test` set
