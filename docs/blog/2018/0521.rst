...
Shortly after releasing 1.1.8, we discovered another minor bug in
atelier.  When the :xfile:`~/.atelier/config.py` file contains an
invalid project name (i.e. it calls
:func:`atelier.projects.add_project` with a `root_dir` that doesn't
exist), the project is being added to the list, but :cmd:`pp -l`
failed::
